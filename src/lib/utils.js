...
export const confirmDialog = confirmObj => {
  ...
  return Swal.fire({
    ...
    customClass: {
      confirmButton:
        'bg-blue-500 border border-blue-500 text-white px-4 py-2 rounded cursor-pointer',
...
export const userData = userDataStr ? (userDataStr.startsWith('{') || userDataStr.startsWith('[') ? JSON.parse(userDataStr) : userDataStr) : null
export const userRole = userData?.role?.name || userData?.role || undefined
...
export const menus = typeof window !== 'undefined' ? getLocal('menus') || [] : []
...
export const getObjectWithValidValues = object => {
  const validObject = {}
...
export const createLink = menu_name => {
