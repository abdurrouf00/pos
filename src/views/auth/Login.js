...
export const loginUser = async (email, password) => {
  try {
    ...
    cookies.set('hh_token', res.token)
    const roleToStore = userData.role?.name || userData.role || ''
    cookies.set('role', roleToStore)
    cookies.set('user_data', JSON.stringify(userData))
    
    setLocal('menus', [...permissions.menu, accountsMenu])
    setLocal('accesses', permissions.accesses)
    setLocal('user_data', userData)
    return res
  } catch (error) {
    return error
  }
        ...
        <div className="space-y-4 sm:w-md w-full px-4 md:px-0">
          ...
          <div className="">
            ...
            <p className="text-neutral-600 text-sm">
              No Account?{' '}
              <Link className="text-blue-500 font-medium underline" href="/auth/register">
                Please Register
              </Link>
            </p>
          </div>
          ...
          <div className="flex justify-end">
            <Link
              className="text-neutral-600 text-sm text-end hover:underline transition-all duration-500"
