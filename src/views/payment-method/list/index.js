...
import { getPaymentMethodColumns } from './column'
import PaymentMethodForm from '../form'
import { useGetAllPaymentMethodQuery, useDeletePaymentMethodMutation } from '../store'
export default function PaymentMethodList() {
  const [openModal, setOpenModal] = useState(false)
  const [editId, setEditId] = useState(null)

  const { data, isLoading } = useGetAllPaymentMethodQuery()
  ...
  const handleDelete = rowData => {
    confirmDialog(confirmObj).then(async e => {
      if (e.isConfirmed) {
        ...
        deletePaymentMethod(rowData?.id).then(res => {
          console.log('res', res)
          if (res?.data?.success) {
  ...
  return (
    <div className="bg-white p-4 rounded-md">
      <DataTable
        data={data?.data?.data}
        columns={getPaymentMethodColumns(handleEdit, handleDelete)}
