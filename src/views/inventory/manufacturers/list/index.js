...
import { useDeleteManufacturerMutation, useGetAllManufacturersQuery } from '../store'
import { getAllManufacturersColumns } from './columns'
import { useGetAllCountryQuery } from '../../currencies/store'
...
export default function ManufacturersList() {
  ...
  const handleDelete = rowData => {
    confirmDialog(confirmObj).then(async e => {
      if (e.isConfirmed) {
        ...
        deleteManufacturer(rowData?.id).then(res => {
          console.log('res', res)
          if (res?.data?.success) {
  ...
  return (
    <div className="bg-white p-4 rounded-md">
      <DataTable
        data={data?.data?.data}
        columns={getAllManufacturersColumns(handleEdit, handleDelete)}
