...
const schema = z.object({
  ...
  code: z.string().min(1, 'Rate is required'),
  phone: z.string().min(1, 'Rate is required'),
  address: z.string().min(1, 'Rate is required'),
  email: z.string().email(),
  is_active: z.boolean(),
})
export default function WarehouseForm({ open, setOpen, editId, setEditId }) {
  ...
  const [create, { isLoading: createLoading }] = useCreatewarehouseMutation()
  const [update, { isLoading: updateLoading }] = useUpdatewarehouseMutation()
  const { data: editData, isFetching: editDataLoading } = useGetwarehouseByIdQuery(editId, {
    skip: !editId,
  })
