...
export default function ContinuousLateReport() {
    ...
    <UILoading loading={loading}>
      {
        details.length === 0 ? <div className='bg-white flex justify-center items-center h-full'>
          ...
          <div ref={targetRef} className='  px-4 py-8 mb-4'>
            <Header title="Continuous Late Report" date={data ? data?.date : ''} />
            <div className='grid grid-cols-2 gap-4 my-4 text-sm'>
              <div>
                <div className='flex  gap-2'>
                  <h4 className=' font-bold'>Employee ID & Name:</h4>
                  <p>{details[0]?.employee?.emp_id} - {details[0]?.employee?.name}</p>
                </div>
                <div className='flex  gap-2'>
                  ...
                  <table className='w-full'>
                    <thead>
                      <tr className='bg-gray-100  text-left '>
                        ...
                        <th className='px-2 '>In Time</th>
                        <th className='px-2 '>Late Duration (Minutes)</th>
                        ...
                        reportData.map((item) => (
                          item.attendance_details.map((detail) => (
                            <tr key={detail.id} className='text-sm border border-gray-50'>
                              ...
                              <td className='px-2 py-1 '>{detail.late_minutes ?? ''}</td>
