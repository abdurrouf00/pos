...
export const updateBranch = createAsyncThunk('branch/updateBranch', async data => {
  const result = axios
    .post(`${branchApi}/${data?.id}`, { ...data, _method: 'PUT' })
    .then(res => {
      return res
    })
    .catch(err => console.log(err))
...
export const getBranchById = createAsyncThunk('branch/getBranchById', async id => {
  const result = axios
    ...
    .then(res => {
      return {
        ...res?.data?.branch,
        has_multiple_shift: res?.data?.branch?.has_multiple_shift ? 1 : 0,
        is_head_office: res?.data?.branch?.is_head_office ? 1 : 0,
        location_attributes: res?.data?.branch?.location_attributes
          ? res?.data?.branch?.location_attributes
          : null,
        distance: res?.data?.branch?.location_attributes?.distance
          ? res?.data?.branch?.location_attributes?.distance
          : '',
        latitude: res?.data?.branch?.location_attributes?.latitude
          ? res?.data?.branch?.location_attributes?.latitude
          : '',
        longitude: res?.data?.branch?.location_attributes?.longitude
          ? res?.data?.branch?.location_attributes?.longitude
          : '',
      }
    })
    .catch(err => console.log(err))
