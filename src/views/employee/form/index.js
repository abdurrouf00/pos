...
import { getAllProjects } from "@/views/project/store";
...
const EmployeeForm = ({ id }) => {
  ...
  const { basicEmployeeData, loading, } = useSelector(
    ({ employee }) => employee
  );
  ...
  const commonErrorSchema = yup.object().shape({
    ...
    role_id: basicEmployeeData.username && !basicEmployeeData.role_id ? yup.string().required("Role is required") : yup.string(),
    email: basicEmployeeData.username && !basicEmployeeData.email ? yup.string().required("Email is required") : yup.string(),
    password: basicEmployeeData.username && !basicEmployeeData.password ? yup.string().required("Password is required") : yup.string().min(6, "Password must be at least 8 characters long"),
  })

  const addSchema = commonErrorSchema

  const updateSchema = commonErrorSchema.shape({
    department_id: basicEmployeeData.department_id ? yup.string() : yup.string().required("Department is required"),
    designation_id: basicEmployeeData.designation_id ? yup.string() : yup.string().required("Designation is required"),
    nid_no: basicEmployeeData.nid_no ? yup.string() : yup.string().required("NID is required"),
    basic_salary: basicEmployeeData.basic_salary ? yup.string() : yup.string().required("Basic salary is required"),
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        const route = id ? `/dashboard/employee/${id}` : `/dashboard/employee`;
        router.push(route);
