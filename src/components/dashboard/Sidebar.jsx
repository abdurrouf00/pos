...
import { cn, getLocal } from "@/lib/utils";
...
const activeClass =
  "bg-gradient-to-r from-gray-100 via-blue-50 to-gray-100 font-bold shadow-md shadow-gray-100 text-blue-800 border-blue-100/40";
...
export default function Sidebar({ open, setOpen }) {
  ...
  useEffect(() => {
    if (isBrowser) {
      try {
        ...
        const numericModuleId = moduleId ? parseInt(moduleId, 10) : null;
        const formattedMenuItems = menus
          ?.filter((ff) => ff.menu_name !== "Dashboard")
          ?.filter((mm) => mm.id === numericModuleId)
          ?.map((dd) => {
            return {
              title: dd.menu_name,
              href: "/dashboard",
  ...
  useEffect(() => {
    if (menuItems.length > 0) {
      ...
      const parentPath =
        ...
        findParentPath(menuItems[0].subItems, pathname);
      if (parentPath) {
  ...
  const renderMenuItem = (item, parentPath = "") => {
    ...
    return (
      <div key={currentPath} className="">
        <div
          className={cn(
            "flex items-center gap-2 py-2.5 px-4 rounded text-sm hover:bg-neutral-100 transition-colors duration-300 cursor-pointer  ",
            {
        ...
        >
          ...
          {hasSubItems ? (
            <button
              ...
              className={cn(
                "flex justify-between items-center w-full  text-[14px] flex-grow  "
              )}
            >
              {item.title}
              <span
            ...
            <span
              href={item.href}
              className={cn("flex items-center  text-[0.8rem] rounded ")}
            >
              {item.title}
            </span>
          )}
  ...
  return (
    <>
      <div
        className={cn(
          "inset-y-0 left-0 z-30 w-64 bg-white shadow-xl transform transition-transform duration-500 ease-in-out overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100 hover:scrollbar-thumb-gray-400 fixed lg:relative",
          {
      ...
      >
        ...
        <Link
          href={"/dashboard"}
          className={cn(
            "group text-neutral-700  items-center mx-5 px-3 py-2 text-[0.8rem] rounded hover:bg-neutral-100 transition-colors duration-300 flex gap-2",
            {
          ...
          {menuItems
            ...
            .map((category) => (
              <div key={category.title} className="mt-1 mx-3">
                ...
                {category.subItems &&
                  category.subItems.map((item) => renderMenuItem(item))}
              </div>
            ))}
        </div>
      </div>
